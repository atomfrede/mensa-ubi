...
@EActivity
public class DetmoldActivity extends AbstractWeeklyMenuActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSpinnerAdapter = ArrayAdapter.createFromResource(this, R.array.detmold_dropdown,
	...
	private void exchangeFragment(int position, boolean initial){
		if(currentPosition != position || initial){
			...
			case 0:
				//Normal, current week
				fmt.replace(R.id.fragment_container, WeeklyMealFragment.newInstance(Constants.DETMOLD_XML_KEY, Constants.detmoldUrl, Constants.LOC_DETMOLD));
				fmt.commit();
				break;
			case 1:
				//Next week, week+1
				fmt.replace(R.id.fragment_container, WeeklyMealFragment.newInstance(Constants.DETMOLD_NEXT_XML_KEY, Constants.detmoldUrlNextWeek, Constants.LOC_DETMOLD_NEXT_WEEK));
				fmt.commit();
				break;
			default:
				break;
	...
	protected void reloadData() {
		try {
			MealPlan.getInstance().setDetmoldMenu(
					Parser.parseMenu(false, settings.getString(Constants.DETMOLD_XML_KEY, null), settings, Constants.detmoldUrl,
							Constants.DETMOLD_XML_KEY));
		} catch (Exception e) {
