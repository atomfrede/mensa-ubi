...
//		getSupportActionBar().setSelectedNavigationItem(currentPosition);
	...
	private void exchangeFragment(int position, boolean initial){
		if(currentPosition != position || initial){
			...
			case 0:
				//Normal, current week
				fmt.replace(R.id.fragment_container, WeeklyMealFragment.newInstance(Constants.HOEXTER_XML_KEY, Constants.hoexterUrl, Constants.LOC_HOEXTER));
				fmt.commit();
				break;
			case 1:
				//Next week, week+1
				fmt.replace(R.id.fragment_container, WeeklyMealFragment.newInstance(Constants.HOEXTER_NEXT_XML_KEY, Constants.hoexterUrlNextWeek, Constants.LOC_HOEXTER_NEXT_WEEK));
				fmt.commit();
				break;
			default:
				break;
