	...
	private static void extractRow(Element dailyBlock, DailyMenu dailyMenu, String dayInformation) {
		...
		for (Element row : rows) {
			...
			if (!importantElements.isEmpty()) {
				...
				String menuText = row.child(1).text();

				if (menuText.trim().endsWith("€")) {
					menuText = menuText.trim().substring(0, menuText.length() - 6);
				}

				menu.setText(menuText);

				Elements priceElements = row.child(3).select("p");
				StringBuilder sb = new StringBuilder();
				int counter = 0;
				...
				if (priceElements.size() == 0) {
					menu.setPrice(row.child(3).text());
				}
